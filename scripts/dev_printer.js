...
if (fs.existsSync(electron_printer)) {
    ...
    if (shell.exec("node-pre-gyp clean configure build --target_arch=x64 --target_platform=win32 --runtime=electron --target=1.2.5 --build-from-source && node-pre-gyp package --target_arch=x64 --target_platform=win32 --runtime=electron --target=1.2.5 --dist-url=https://atom.io/download/atom-shell", {silent:true}).code !== 0) {
